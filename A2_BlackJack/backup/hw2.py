...
class BlackjackMDP(MDP):
    ...
    def __init__(self): 
        self.terminal_state = ('terminal', 0, 0, 0, 0)
        self.start_state = ('start', 0, 0, 0, 0)
    ...
    def get_new_hand_value(self, hand, ace, card, phase):
        ...
        # if so then adjust the hand value and reduce number of useable aces
        if hand > 21:
            if ace > 0:
                hand -= 10
                ace -= 1
    ...
    def get_succ_states(self, state):
        ...
        if phase == 'player':
            for n in range(1, 11):
                ...
                player_hand, player_ace, prob = self.get_new_hand_value(state[1], state[2], n, phase)
...
def main():
    ...
    for state in mdp.states:
        if state[0] == 'dealer' and state[3] == 13 and state[4] > 0:
            count += 1
            print(state)
    print('-------')
    print(f'states where dealer has hand value 12 and an ace: {count}')
    print('-------')
